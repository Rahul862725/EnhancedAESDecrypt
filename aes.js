...
const aesEncy1 = (msg, key,iv) => {
 c=0
    ...
    {
        binaryData = binary(encData[i].charCodeAt(0))
        hexaText += convertHex(binaryData.substr(0,4))+convertHex(binaryData.substr(4,4))
    }
...
const aesDec1 = (msgHex, key,iv) => {
    ...
    {
        msg+= String.fromCharCode(decimal(HexaToBinary(msgHex[i])+HexaToBinary(msgHex[i+1])))
    }    
    let decData =""
    for(let i=0;i<msg.length/16;i++)
    {
